...
const validateAddress = (pickupId, dropoffId) => {
  ...
  if (dropoffAddress.length < 5) {
    displayError(dropoffId, "L'adresse de livraison est trop courte.");
    return false;
  }
  if (pickupAddress === dropoffAddress) {
    displayError(
      dropoffId,
      "Les adresses de ramassage et de livraison ne peuvent pas être identiques."
    );
    return false;
  }
...
const validateMovingDate = (id) => {
  ...
  if (movingDate < today) {
    displayError(
      id,
      "La date de livraison  ne peut pas être antérieure à aujourd'hui."
    );
    return false;
  }
  displayError(id, "", "success");
